...
## Table 1 ##
table(subset(data_part, oecd==0)$partner, I(subset(data_part, oecd==0)$prod2>0)) #Table 1: partnerships 
#Notes. 1: China's partner; 0: Non-partner; True: Oil producer; False: Non-oil producer

table(I(data_aid$aid2)>0, I(data_aid$prod2>0)) #Table 1: aid to Africa
#Notes. True (row): China's aid recipient; False (row): Non-aid recipient; True (column): Oil producer; False (column): Non-oil producer

table(I(subset(data_visit, oecd==0)$visit)>0, I(subset(data_visit, oecd==0)$prod2>0)) #Table 1: state visits
...
box()
legend("topleft", c("China's oil consupmtion (KBPD)", "# of China's partnerships"), cex=1.3, lwd=4, lty=c(1, 6))
...
box()
legend("topleft", c("China's oil consupmtion (KBPD)", "Chinese aid to Africa (millions)"), cex=1.3, lwd=4, lty=c(1, 9))
mtext(side=1, "Year", line=2.5, cex=1.5)
