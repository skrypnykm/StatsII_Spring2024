...
\vspace{.25cm}
	...
	\lstinputlisting[language=R, firstline=55, lastline=57]{PS2MS.R} 
	...
	\lstinputlisting[language=R, firstline=59, lastline=61]{PS2MS.R} 
...
To establish whether at least one predictor is a significant predictor in our logistic regression model, we should compare the full model to the reduced model. We can use \texttt{anova()} function to perform a likelihood ratio test.
	...
	\lstinputlisting[language=R, firstline=63, lastline=67]{PS2MS.R} 
...
\lstinputlisting[language=R, firstline=70, lastline=71]{PS2MS.R} 
...
\lstinputlisting[language=R, firstline=73, lastline=75]{PS2MS.R} 

\begin{verbatim}
 0.7224531
\end{verbatim}

\lstinputlisting[language=R, firstline=77, lastline=79]{PS2MS.R} 
...
\[ P(Y_i = 1|X_i) = \frac{e^{\beta_0 + \beta_1 \cdot 1 + 0 \cdot X_{\text{sanctionsnone}}}}{1 + e^{\beta_0 + \beta_1 \cdot 1 + 0 \cdot X_{\text{sanctionsnone}}}} = \frac{1}{1 + e^{-(\beta_0 + \beta_1)}} \]
	...
	\lstinputlisting[language=R, firstline=81, lastline=84]{PS2MS.R} 
	\begin{verbatim}
		 0.5159191 
	\end{verbatim}

	\lstinputlisting[language=R, firstline=86, lastline=88]{PS2MS.R} 
...
\textbf{First}, let's run an interactive logistic regression model: 
	...
	\lstinputlisting[language=R, firstline=90, lastline=94]{PS2MS.R} 
...
Even at a glance, we can see that the coefficients for all the interactive terms are not statistically significant, which can give us a notion that adding interaction to the model does not vastly contribute to explaining the variability in the response variable  \texttt{choice}. Let's perform a likelihood ratio test to support or reject this hypothesis:
	...
	\lstinputlisting[language=R, firstline=97, lastline=99]{PS2MS.R} 
...
Given the \textbf{p-value} of 0.3912, we have \underline{not} found evidence to reject the null hypothesis (adding an interaction to the model does not significantly improve its fit) at the $\alpha$ level = 0.05. This suggests that the association between the response variable and the predictor variables can be explained by the main effects of the predictors alone, without considering their interactions. This can also be supported by the fact that the coefficients for most of the interaction terms are not statistically significant.
	
Furthermore, including the interaction terms would potentially change the answers both to 2(a) and 2(b):
\begin{itemize}
	\item \textbf{2(a)}: A given $\beta$ change in  \texttt{sanctions} may not have the same effect on the odds of an individual supporting the policy (\texttt{choice} = 1), as it may now be dependent on the specific number of \texttt{countries}  participating in the international agreement ('levels' of the other explanatory variable). We can see that in the new model, the coefficients of interest have changed; therefore, the effect of increasing sanctions from 5\% to 15\% would change as well:
		\lstinputlisting[language=R, firstline=101, lastline=104]{PS2MS.R} 
	...
	\lstinputlisting[language=R, firstline=106, lastline=108]{PS2MS.R} 
	...
	Calculating the difference of two probabilities: 
		...
		\lstinputlisting[language=R, firstline=111, lastline=111]{PS2MS.R} 
